...
def employee_report(request):
    ...
    if request.user.is_staff:
        ...
        employees = Employee.objects.annotate(
            total_production=Sum('production__quantity')
        )
...
def supplier_report(request):
    ...
    if request.user.is_staff:
        ...
        total_quantity = suppliers.aggregate(
            total=Sum('Quantity')
        )['total'] or 0

        total_price = suppliers.aggregate(
            total=Sum('Price')
        )['total'] or 0
...
def product_report(request):
    ...
    if request.user.is_staff:
        ...
        total_product_price = products.aggregate(
            total=Sum('price')
        )['total'] or 0
...
def individual_employee_report(request, sl):

    # Get employee
    employee = get_object_or_404(Employee, sl=sl)
    ...
    if employee_profile:
        ...
        total_out = attendances.filter(
            status=Attendance.OUT
        ).count()
    ...
    return render(
        request,
        'individual_employee_report.html',
        context
    )
# =========================================================
...
def individual_supplier_report(request, sl):
    ...
    return render(
        request,
        'individual_supplier_report.html',
        context
    )
...
def individual_product_report(request, sl):
    ...
    return render(
        request,
        'individual_product_report.html',
        context
    )